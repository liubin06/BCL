...
# ecpirically estimating AUC
def auc(net, memory_data_loader):
    net.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--beta', default=1.0, type=float, help='Concentration Parameter valued in [0.5,1]')
    parser.add_argument('--alpha_setting', default=None, type=str, help='Adaptive alpha method')
    ...
    alpha = args.alpha
    beta = args.beta
    alpha_setting = args.alpha_setting
    ...
    for epoch in range(1, epochs + 1):
        if (epoch-1) % 40 == 0:
            if alpha_setting == "linear":
                ...
                alpha = auc(model, memory_loader)
        print("alpha %s" % alpha,"beta %s" %  beta)
        train_loss = train(model, train_loader, optimizer, temperature, estimator, tau_plus, alpha, beta)

        if epoch >= 300:
            if epoch % 20 == 0:
                torch.save(model.state_dict(),
                           '../results/{}/{}_{}_model_{}_{}_{}_{}.pth'.format(dataset_name, dataset_name, estimator, batch_size, alpha, beta, epoch))