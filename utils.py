...
def get_dataset(dataset_name, root='../data', pair=True):
    if pair:
        if dataset_name=='cifar10':
            ...
            memory_data = CIFAR100Pair(root=root, train=True, transform=test_transform,download=True)
            test_data = CIFAR100Pair(root=root, train=False, transform=test_transform,download=True)
        elif dataset_name=='stl10':
            train_data = STL10Pair(root=root, split='train+unlabeled', transform=train_transform,download=True)
            memory_data = STL10Pair(root=root, split='train', transform=test_transform,download=True)
            test_data = STL10Pair(root=root, split='test', transform=test_transform,download=True)
